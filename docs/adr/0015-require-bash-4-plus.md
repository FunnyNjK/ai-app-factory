...
**2. Each executable entry point self-re-execs under a modern bash.** A new sourced helper, `scripts/orchestrator/require-bash4.sh`, runs as the first action of every adapter (right after `SCRIPT_DIR` is resolved, before any bash-4-only syntax is parsed). If the current interpreter is bash < 4, it locates a bash ≥ 4 (checking `/opt/homebrew/bin/bash`, `/usr/local/bin/bash`, `/opt/local/bin/bash`, then `PATH`) and `exec`s the calling script under it with the original arguments. Loop safety comes from the version check itself: a candidate is exec'd only after proving `BASH_VERSINFO >= 4`, so on re-entry the guard is a no-op. This works because bash 3.2 executes a script's leading simple commands *before* it parses the later heredoc that it would choke on — so the re-exec happens first. The helper itself is kept strictly bash-3.2-compatible.

> **Amendment (2026-06-09).** The first version also latched an exported `FACTORY_BASH_REEXEC=1` as an idempotency flag. That was a defect: exported variables are inherited by child processes, so the re-exec'd orchestrator passed the flag to every adapter it spawned, whose own guards then **skipped** the re-exec and ran on bash 3.2 — re-creating the exact crash this ADR exists to prevent (observed live on `simplytammi` slice 3.1; masked the previous day only because the operator's command still carried a `PATH=` prefix). The flag is removed; each script decides from its own interpreter version alone, which the pre-exec version probe already makes loop-safe.
