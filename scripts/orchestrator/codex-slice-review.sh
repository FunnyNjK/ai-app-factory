...
You are ${ROLE_NAME}, the Quality Engineer for this AI App Factory project. Your task this session: review slice ${SLICE_ID} from TASKS.md.
...
3. Read the slice's acceptance criteria. If the slice's TASKS.md acceptance line (or the ARCHITECTURE.md section it points to) names a consolidated per-slice acceptance checklist with stable item ids (ADR-0014 — an explicit, enumerable list such as 2.4-AC-01, 2.4-AC-02, ...), THAT checklist is the authoritative gate; load it and use its items verbatim. Otherwise read the criteria from ARCHITECTURE.md Work Breakdown and CURSOR_HANDOFF.md.
4. Read the recent git diff for changes Cursor made to implement this slice (use \`git log --oneline -n 5\` and \`git diff HEAD~1\`).
5. Review EXHAUSTIVELY in this one pass — do not stop at the first defect. Build the complete acceptance-criteria list FIRST: if a consolidated checklist exists (step 3), use its items verbatim; otherwise enumerate every criterion from ARCHITECTURE.md Work Breakdown, CURSOR_HANDOFF.md, API_SPEC.md, and TEST_PLAN.md (every status code, error envelope, validation rule, header, log field, and edge case the contract names) and treat that enumeration as the list. Then assign EVERY item exactly one verdict: covered (name the passing test that proves it), gap (a sub-task is needed), or n/a (one-line reason it does not apply). Run all tests Cursor added and confirm they pass; verify each item not yet test-covered. Also apply the per-project-type checklist in factory AGENTS.md (security smoke, accessibility baseline as applicable, integration points, error handling). Decide ONLY after every item has a verdict — filing gaps a few at a time across multiple rounds is the failure mode this guards against (it has exhausted the per-task iteration cap on prior slices). Include the full per-item verdict list (item id -> covered/gap/n/a) in your response, AND append a compact verdict line to the slice's Notes in TASKS.md, e.g. Verdicts (round 2): AC-01 covered (tests/unit/x.test.ts), AC-02 gap -> ${SLICE_ID}.d, AC-03 n/a (JSON API). VERDICT STABILITY: if the slice's Notes already carry a verdict line from a prior round, prior covered/n-a verdicts are SETTLED. Re-open a settled item ONLY if (a) the implementation relevant to that item changed since that round, or (b) the prior verdict was factually wrong — and in either case the sub-task text must name the prior verdict and why it no longer holds. Do not re-litigate a settled item by raising the bar (e.g. demanding stricter assertions for an item already verdicted covered) — demand the exact assertions you need in the round where you first verdict the item.
6. Decide:

   (A) APPROVED — every checklist item is covered or n/a, all tests pass, no significant defect:
   ...
   (B) SUB-TASKS NEEDED — one or more defects, missing tests, or contract gaps:
       - File one sub-task for EVERY item you marked gap in step 5 now, in this single review — do not defer some to a later round. Reference the failing item id where the slice has a consolidated checklist. Update TASKS.md: set slice ${SLICE_ID} Status to in-progress, append numbered sub-tasks to the slice's Sub-tasks line (use ${SLICE_ID}.a, ${SLICE_ID}.b, ...). Each sub-task must be specific (file/function/line) and testable.
...
log "Invoking $TOOL for slice $SLICE_ID review (role '$ROLE_NAME', wall-time cap ${WALL_TIME}s)..."
set +e
