...
1. **Project Classification** — what type of project this is (marketing site, full-stack app, API service, Azure Functions workflow, Stripe-enabled, Plaid-enabled, internal tool, etc.). If the type or complexity is unclear, walk `docs/choosing-a-blueprint.md` to converge on a base blueprint, any overlays, and a starting tier. If still unsure, ask one clarifying question.
